...
- **e2e (CDP): 69 skipped** — those same 19 placeholders **plus 50 tests that cannot run against the real service**:
  ...
  - **22** — `no-role` / `no-projects` profile tests (role enforcement → `/auth/forbidden`, empty state) that only the stub can mint.
