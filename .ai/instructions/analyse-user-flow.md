...
Produce or update a `test/flows/<flow-name>.flow.md` file that reflects the current state of the named user flow in the frontend and backend source. Flow docs are the single source of truth consumed by `/discover-journey-tests` and `/validate-ac-automated`.
...
- Populate **On success** and **On error** from the controller's redirect and view calls. These tell `/discover-journey-tests` what journeys to assert, and give the backend-coverage check in `.ai/instructions/coverage-boundaries.md` the response codes and behaviours to look for in the sibling suites.
