...
// BMD-405 copy that cannot be reached today: every current fixture with the
// named defect also trips side errors (distinctiveness, area-sum-mismatch,
// slivers), so the multi-error layout renders instead (those gates are covered
// in describeGeometricGateErrors above). Each entry needs a new single-defect
// fixture — its parcels must tile the redline exactly and use in-scope
// distinctiveness. To enable one:
//   1. add the single-defect fixture to test/example-files/ under the name
//      given in `fixture`
//   2. move the entry up into SINGLE_ERROR_CASES
