...
- **Backend content validation (post-intervention specifics):** the validate route runs the same pipeline as baseline but with `projectDocumentKey: 'postIntervention'`, which changes:
  - **Filename rule (BMD-958, frontend PR#236 / backend PR#280, 2026-08-24)** — checked before any content validation, and identical for both upload types. Filename must be **≤ 255 characters** and match `/^[a-z0-9][a-z0-9 ._()-]*\.gpkg$/i` — starting alphanumeric, ending `.gpkg`, otherwise letters, digits, spaces, dots, underscores, hyphens and **brackets**. BMD-958 **added `(` and `)`**, so `site survey (final).gpkg` is now accepted where it previously rejected. Failure yields the new `INVALID_FILENAME` code rather than `INVALID_FILE_METADATA`, split out by `makeMetadataError` so the one problem the user can actually fix — rename the file — gets its own copy.
...
**`standard` does not imply a fixed H1.** Most standard entries use the shared `GEOPACKAGE_ERROR_H1` ("Your Geopackage (.gpkg) file contains an error"), but the variant only fixes the _shape_ — H1 plus one sentence plus the inline link. `GPKG_TOO_MANY_FEATURES` is a standard-variant entry with its own H1, and the ref-titled entries substitute a parcel reference. Assert the H1 per code from the table below, not from the variant.
...
| **`INVALID_FILENAME`** (BMD-958)                                                                                           | "Your Geopackage (.gpkg) file contains an error"                           | **"The file name contains characters we cannot accept."** — closes with **"upload the renamed file"**, the only entry overriding the shared link text                                                                                 |
| **`GPKG_TOO_MANY_FEATURES`** (BMD-869)                                                                                     | **"Your Geopackage (.gpkg) file contains too many features"** — its own H1 | **"This file contains {n} features. This service can check up to {max}."** (en-GB thousands separators) — degrades to "This file contains more features than this service can check." when the counts are absent from `error.details` |
