...
- **Description:** Two-section stacked page, H1 = parcel ref, caption = project name.
  ...
  - **Standard time to target condition** (`formatStandardTimeToTarget`,
    ...
    **Frontend PR#206 (BMD-706, 2026-08-13) widened "absent":** the GeoPackage sentinels
    ...
    created branch needs a parcel whose Retention Category is literally `Created` with its
    `Baseline*` columns cleared: `Post-intervention - created area habitat.gpkg` (H2-7,
    `Good - 5 years`), generated by `test/example-files/fixture-mutations.py`. The
    `N/A`-sentinel branch has no journey coverage yet.
